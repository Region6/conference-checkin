    ...
    ctrl.execute(() => {
      ...
      if (
        ...
        if (response.getTransactionResponse() != null && response.getTransactionResponse().getErrors() != null) {
          retval = response.getTransactionResponse().getErrors().getError();
				}
				else {
          retVal = response.getMessages().getMessage();
				}
      }
...
const getTransaction = (trans) => {
  return new Promise((resolve, reject) => {
    ...
    const getRequest = new ApiContracts.GetTransactionDetailsRequest();
	  ...
	  getRequest.setTransId(trans.transactionId);
  ...
  if (transaction) {
    ...
    if (existTransaction.length) {
      results = await db('transactions')
        ...
        .catch(e => console.error('db', 'database error', e));
    } else {
      results = await db('transactions')
        .insert(record)
        .then(
    ...
    console.error('Missing Transaction:', transaction);
  }
  return results;
}
...
const getBatchDates = async () => {
  ...
  const dates = await db('onsiteAttendees')
    ...
    .catch(e => console.log('db', 'database error', e));
  */
 const transactions = await db('registrantTransactions')
  .whereNot(
    {
      type: 'check',
      transactionId: '',
